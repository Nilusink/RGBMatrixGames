...
void displayDateTimePage(PDisplayDevice displayDevice,PRTC_tm pt_RTC_tm,uint8_t *temp)
{
	...
	select_area_color(0,0,45,7,Green,HIGH_ROW);//年月日，绿色
	...
	select_area_color(46,0,63,9,Yellow,HIGH_ROW);//星期，黄色
	...
	select_area_color(1,6,43,15,Purple,LOW_ROW);//农历月日，紫色
	...
	select_area_color(42,6,62,15,Blue,LOW_ROW); //温度显示，蓝色
...
void displaySettingPage(PDisplayDevice displayDevice,uint8_t Select_setting_options)
{
	...
	if(LanguageFlag == CHINESE)
	{
		displayDevice->SetPixel(displayDevice,1,0,0 ,DAT16X16);//设 form font.h
		displayDevice->SetPixel(displayDevice,17,0,1 ,DAT16X16);//置
		displayDevice->SetPixel(displayDevice,33,0,2 ,DAT16X16);//列
		displayDevice->SetPixel(displayDevice,48,0,3 ,DAT16X16);//表
		if(Select_setting_options == 1)
		{
			displayDevice->SetPixel(displayDevice,2,16,4,DAT16X16);//时
			displayDevice->SetPixel(displayDevice,17,16,5 ,DAT16X16);//间
			displayDevice->SetPixel(displayDevice,32,16,0 ,DAT16X16);//设
			displayDevice->SetPixel(displayDevice,48,16,1 ,DAT16X16);//置
		}
		else if(Select_setting_options == 2)
		{
			displayDevice->SetPixel(displayDevice,3,16,6 ,DAT16X16);//日期设置
		...
		{
			displayDevice->SetPixel(displayDevice,3,16,8,DAT16X16);//自动亮度
		...
		{
			displayDevice->SetPixel(displayDevice,1,16,12,DAT16X16);//蜂鸣设置
		...
		{
			displayDevice->SetPixel(displayDevice,1,16,14,DAT16X16);//语言设置
...
uint8_t get_month_date(uint16_t year_cnt,uint8_t month_cnt)//判断一个月的最大天数
{
...
uint8_t get_weekday(uint16_t year_cnt,uint8_t month_cnt,uint8_t date_cnt)//根据年月日判断星期几
{
    uint8_t weekday = 8;
